...
class ScmABCExcelReportController(http.Controller):
    ...
    def get_master_list_mc_report(self, wizard=None, **args):
        ...
        cur.execute('''select a.id, d.name as company, a.barcode, a.default_code, b.name as product, b.brand as brand,
                            c.val_cost_price as cost
                            from product_product a, product_template b, res_company d, stock_production_lot c                           
                            where b.id = a.product_tmpl_id
							and b.id = c.product_id
                            and d.id = c.company_id                           
                            and c.company_id = 2
							and c.val_cost_price is not null
							and c.val_cost_price <> 0
                            group by d.name,product,a.barcode, cost, brand,
							a.default_code,a.id order by a.id asc''')

        # display the PostgreSQL database server version
        masterlist = cur.fetchall()
        for data_m in masterlist:
            print(data_m)
            # the report content
            sheet.write(row, 0, data_m[2], text_style)
            sheet.write(row, 1, data_m[4], text_style)
            sheet.write(row, 2, '', text_style)
            sheet.write(row, 3, data_m[5], text_style)
            sheet.write(row, 4, data_m[6], text_style)
